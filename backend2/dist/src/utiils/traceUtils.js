...
exports.parseTraceFromFile = void 0;
...
const parseTraceFromFile = async (traceZipPath, outputDir) => {
    fs.mkdirSync(outputDir, { recursive: true });
    // Extract trace.zip
    await fs
        .createReadStream(traceZipPath)
    ...
    console.log("🔍 Total trace entries parsed:", lines.length);
    const errorIndex = lines.findIndex((entry) => entry.metadata?.error?.message || entry.error?.message);
    if (errorIndex === -1) {
        throw new Error("No error found in trace.");
    }
    const errorMessage = lines[errorIndex].metadata?.error?.message ||
        lines[errorIndex].error?.message ||
        "Unknown error";
    const testName = lines[errorIndex].metadata?.title || "Unknown Test (no title in trace)";
    const traceContext = lines
        .slice(Math.max(0, errorIndex - 20), errorIndex + 1)
        .map((entry) => JSON.stringify(entry).slice(0, 300));
    ...
    if (networkFile) {
        ...
        if (failedRequests.length > 0) {
            ...
            failedNetworkRequests = failedRequests.map((r) => JSON.stringify(r).slice(0, 300));
        }
    }
    return {
        testName,
...
exports.parseTraceFromFile = parseTraceFromFile;
